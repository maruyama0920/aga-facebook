...
function truncateForMessenger(text, maxChars) {
...
function readMessengerImageUrl(envKey) {
...
// 診断開始
async function startDiagnosis(senderId) {
    userStates.set(senderId, { step: "Q1", answers: {} });
    await sendQuestion1(senderId);
}
// 質問1
async function sendQuestion1(senderId) {
...
async function handleQ1Answer(senderId, payload) {
...
// 質問2
async function sendQuestion2(senderId) {
...
async function handleQ2Answer(senderId, payload) {
...
// 質問3
async function sendQuestion3(senderId) {
...
async function handleQ3Answer(senderId, payload) {
...
// 質問4
async function sendQuestion4(senderId) {
...
async function handleQ4Answer(senderId, payload) {
...
// 質問5
async function sendQuestion5(senderId) {
...
async function handleQ5Answer(senderId, payload) {
...
// 質問6
async function sendQuestion6(senderId) {
...
async function handleQ6Answer(senderId, payload) {
...
// 診断結果誘導
async function sendResultPrompt(senderId, q6Answer) {
...
// 診断結果表示
async function showDiagnosisResult(senderId, payload) {
...
// 診断結果1: プライバシー重視
async function showPrivacyResult(senderId) {
    const hero = readMessengerImageUrl("IMAGE_RESULT_PRIVACY");
    if (hero)
        await sendImageMessage(senderId, hero);
...
// 診断結果2: 効果重視
async function showEffectResult(senderId) {
...
// 診断結果3: 価格重視
async function showPriceResult(senderId) {
...
// カウンセリング誘導
async function sendCounselingPrompt(senderId) {
...
// もっと知りたい
async function showMoreInfo(senderId) {
...
// 連絡先入力リクエスト
async function requestContact(senderId) {
...
// 連絡先入力処理
async function handleContactInput(senderId, text) {
...
function validatePhoneNumber(phoneRaw) {
    // 全角数字を半角に変換
    const normalized = phoneRaw.replace(/[０-９]/g, (s) => String.fromCharCode(s.charCodeAt(0) - 0xfee0));
    // ハイフンを除去
...
// ===== メール送信 =====
// メール送信
async function sendEmailNotification(senderId, name, phone, state) {
...
// メール本文のフォーマット
function formatMailBody(senderId, name, phone, state) {
    ...
    if (state?.answers) {
        body += `=== 診断結果 ===\n`;
        if (state.answers.q1)
            body += `質問1: ${getAnswerText(state.answers.q1)}\n`;
        if (state.answers.q2)
            body += `質問2: ${getAnswerText(state.answers.q2)}\n`;
        if (state.answers.q3)
            body += `質問3: ${getAnswerText(state.answers.q3)}\n`;
        if (state.answers.q4)
            body += `質問4: ${getAnswerText(state.answers.q4)}\n`;
        if (state.answers.q5)
            body += `質問5: ${getAnswerText(state.answers.q5)}\n`;
        if (state.answers.q6)
            body += `質問6: ${getAnswerText(state.answers.q6)}\n`;
    }
    return body;
}
...
async function sendGenericTemplate(recipientId, element) {
    try {
        const el = {
...
async function sendInteractiveCard(recipientId, args) {
    if (args.imageUrl) {
        ...
        const text = args.subtitle?.trim() ? `${args.title}\n\n${args.subtitle}` : args.title;
